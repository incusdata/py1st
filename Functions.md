...
Functions are created with the [**def** keyword][p-st-def], with a name ([ident]{.stx}ifier), followed by parentheses enclosing an optional argument list ([args]{.stx}), followed by a [block]{.stx}, which constitutes the ‘body’ of the function.

Like [assignment][p-st-assign], [**def**][p-st-def] creates a name, and associates that name with the code ‘value’. Multiple names can reference the same function, just like multiple names can store references to the same list, tuple, dictionary, etc.

A function's [block]{.stx} must contain at least one statement, even if it is just a null statement ([**pass**][p-st-pass]).

#### **Syntax** — *Function Definition*

&nbsp;&nbsp;&nbsp;&nbsp; [**def**][p-st-def] [ident]{.stx} **(** [ [args]{.stx} ] **):**  
&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; **"""** [[docstring][p-gl-docstring]]{.stx} **"""**  
&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; [block]{.stx}

&nbsp;&nbsp;&nbsp;&nbsp; [**def**][p-st-def] [ident]{.stx} **(** [ [args]{.stx} ] **):**  
&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; **"""** [[docstring][p-gl-docstring]]{.stx} **"""**  
&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; [block]{.stx}  
&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; [**return**][p-st-return] [ [expr]{.stx} ]

&nbsp;&nbsp;&nbsp;&nbsp; [**def**][p-st-def] [ident]{.stx} **(** [ [args]{.stx} ] **):** [statement]{.stx}

&nbsp;&nbsp;&nbsp;&nbsp; [**def**][p-st-def] [ident]{.stx} **(** [ [args]{.stx} ] **):** [**return**][p-st-return] [ [expr]{.stx} ]
...
Also be sure to notice that the [expr]{.stx} part after [**return**][p-st-return] is optional, in which case the function will still immediately return to the caller, but the return value will be [**None**][p-lit-none].

&nbsp;&nbsp;&nbsp;&nbsp; [**return**][p-st-return] &nbsp;&nbsp; ≡ &nbsp;&nbsp; [**return**][p-st-return] [**None**][p-lit-none]
...
Docstrings are stored in the **\_\_doc\_\_** attribute of modules, classes and functions. Tools like [**pydoc**][p-lib-pydoc] and the built-in [**help**][p-fn-help] function will read and format the string in **\_\_doc\_\_**, which by default is [**None**][p-lit-none].

The [**doctest**][p-lib-doctest] allows you to write [tests][w-tests] as part of the docstring, in the form of example calls with expected results.

| &#x23FA; **NOTE** — **Long Strings as DocStrings**     |
|:--------------------------------------------|
...
To get documentation for a [module]{.stx} and all its classes and functions, use [**pydoc**][p-lib-pydoc]:

&nbsp;&nbsp;&nbsp;&nbsp; **python** **-m** **pydoc** [module]{.stx}

On Unix-like operating system, [**pydoc**][p-lib-pydoc] will be an executable ‘launcher’:

&nbsp;&nbsp;&nbsp;&nbsp; **pydoc** [module]{.stx}

It can also display documentation for your own [script]{.stx}**.py** files:

&nbsp;&nbsp;&nbsp;&nbsp; **python** **-m** **pydoc** [script]{.stx}<br/>
&nbsp;&nbsp;&nbsp;&nbsp; **pydoc** [script]{.stx}
...
All functions return exactly one *result*. Functions return [**None**][p-lit-none] by default. You control return results with ‘[**return**][p-st-return] [expr]{.stx}’, though the [expr]{.stx}ession is optional, in which case Python inserts [**None**][p-lit-none].

Multiple [**return**][p-st-return] statements may appear in a function, as long as you remember that no further code in the function will execute — it is an execution transfer statement, just like [**break**][p-st-break] and [**continue**][p-st-cont].
...
#### **py** — *Function returning a result*
```py
def foo ():
    """
...
Wherever the function **foo** is called, the call expression will *result* in whatever the function [**return**][p-st-return]ed. You do not necessarily have to assign its return value to a variable, unless you want to save it for later use.

A function that has multiple [**return**][p-st-return] statements is shown below. For variety, it takes an argument, but that is not relevant to the topic. Notice the use of the [**is** operator][p-op-is] to test the [**type**][p-fn-type] of the argument.

[p-op-is]:
   https://docs.python.org/3/reference/expressions.html#is-not
   "Python Reference — Expressions - Identity comparisons"

#### **py** — *Function containing multiple return statements*
```py
def foo (param):
    """
...
Note that [**else**][p-st-if], or [**elif**][p-st-if] was not necessary, since [**return**][p-st-return] will return immediately. If the [**True**][p-lit-true] block of one [**if**][p-st-if] was not executed, the next [**if**][p-st-if] executes.
...
#### **py** — *Parameters, with arguments passed positionally*
```py
def three(a, b, c):
    """
...
When a function is called, the arguments can be *named*. This is referred to as **[keyword arguments][p-gl-kwargs]**. For positional parameters like the **three** function above, when *all* arguments are named, it does not matter in which order they appear in the function call.

[p-gl-kwargs]:
   https://docs.python.org/3/glossary.html#term-argument
   "Python Reference — Glossary — arguments"

#### **py** — *Passing keyword arguments*
```py
...
#### **py** — *Mixing positional and keyword arguments*
```py
...
#### **py** — *Various examples of passing arguments*
```py
...
A special syntax allows a parameter of a function to be a [**tuple**][p-fn-tuple] of values. It does not matter what you pass, it will always be a [**tuple**][p-fn-tuple]. This allows callers to call the function with a variable number of arguments. If no arguments are passed, it is effectively an empty [**tuple**][p-fn-tuple]. 

#### **py** — *Function with variable number of arguments*
```py
def five (*parms):
    """
...
#### **py** — *Function with variable, and named/optional, parameters*
```py
...
For interest's sake, this is how the [**print** function][p-fn-print] has been defined (with different parameter names, of course).
...
Just like **\*parms** provides for a [**tuple** type][p-fn-tuple] parameter, it is possible to define a parameter with a leading **\*\***, in which case it will always be a [**dict**][p-fn-dict]ionary. This means that the arguments to be collected in such a parameter, *must* be passed as keyword arguments. It can be combined with a **\*parms** list argument, but then it must appear after it.

#### **py** — *Function with keyword dictionary parameter*
```py
call_count = 0
...
#### **py** — *Function with variable, and keyword, parameters*
```py
call_count = 0
 ...
 * As a consequence of the ‘**/**’ operator, **a** and **b**, can only be passed *positionally*.

 * The **c** and **d** parameters can be passed as positional arguments, *or* as keyword arguments.

 * The **\*** operator dictates that **e** and **f** can only be passed as *keyword arguments*.

#### **py** — *Special parameter operators example*
```py
def F(a, b, /, c, d, *, e, f):
   print(a, b, c, d, e, f)
...
#### **py** — *Invalid arguments violating argument operators*
```py
...
This is really rather tricky and specialised, since it ‘fine-tunes’ the *design* of a function. But *users* of a function, have to understand what these operators mean, in order to call the function correctly. The rationale for ‘positional only’ (**/**) arguments is documented in [PEP 570][pep570].
...
Python implements scope with dictionaries. A reference to a function's ‘local scope’ can be obtained with the built-in [**locals**][p-fn-locals] function.
...
We can create ‘local names’ using any of the normal ‘name-creation statements’: [assignment][p-st-assign], [**def**][p-st-def], and [**class**][p-st-class]. This means we can have local variables, local functions, and local types (classes).
...
Local variable and parameter names ([ident]{.stx}ifiers) share the same ‘namespace’, which means their names are stored in the same dictionary. This dictionary can be accessed using the built-in [**locals**][p-fn-locals] function, which returns reference to a ‘snapshot’ of the local namespace. A new local dictionary is created each time the function is called, and it is cleared or ‘goes out of scope’ when the function returns.

#### **py** — *Function local dictionary entries*
```py
def foo (param):
...
Identifiers created local to a function, are only visible in that function. The [**locals**][p-fn-locals] function is only useful when used *inside* a function. When the [**locals**][p-fn-locals] function is used in the global environment, it will be equivalent to a call to the [**globals**][p-fn-globals] function. Calling the built-in [**vars**][p-fn-vars] function *without* an argument inside a function, will be equivalent to calling [**locals**][p-fn-locals] inside the function.
...
#### **py** — *Abstraction of algorithms as local functions*
```py
def calc_stats(data):
...
#### **py** — *Local function for repetitive code*
```py
...
#### **py** — *Access global variable in function*
```py
...
If, for some reason, a function wants to *modify* a global variable, the [**global**][p-st-global] statement can be used. This informs Python of the global names you want to *modify* with assignment, and to not create a new name locally.

#### **py** — *Global statement prevents local name generation*
```py
...
If **gvar** did not exist at the time when **foo** was called, the ‘**gvar = 456**’ statement would have created the **gvar** globally! The first [**print**][p-fn-print] above would have failed, but the second [**print**][p-fn-print] would still output **456**.

### Outer Names

There is one more statement to deal with: the [**nonlocal**][p-st-nonlocal] statement. You might be thinking that if a name is not local, it must be global. But that is not necessarily true. Consider an **inner** function, nested in an **outer** function.

The **outer** function defines some **name** locally. The **inner** function wants to *modify* that **name**. But assignment *creates* names in the current scope. We need a way to specify that the **inner** function wants to *modify* the **name** in the **outer** function. And thus the need for the [**nonlocal**][p-st-nonlocal] statement.

#### **py** — *Non-local referring to ‘outer’ names*
```py
def outer ():
   name = "value"
...
Note that [**global**][p-st-global] would not have worked… ‘**name = name[::-1]**’ would run, but create a new **name** in the *global scope*.

And that is how scope works in Python: nested dictionary lookups, which may require special handling using the [**global**][p-st-global] and [**nonlocal**][p-st-nonlocal] statements.
...
Functions are [**callable**][p-fn-callable] [**first-class**][wp-fcf] objects created with the [**def**][p-st-def] keyword. Functions defined inside a class are called *methods*.

[wp-fcf]:
   https://en.wikipedia.org/wiki/First-class_function
   "Wikipedia — First-class functions"

Ultimately, a function is an *object* that is [**callable**][p-fn-callable]. This ‘callable value’ can be associated with an [ident]{.stx}ifier, which is what [**def**][p-st-def] does. It can be stored in sequences, passed as argument, or returned from functions… just like you can do with *any* value of *any* type; in other words: **any** [expr]{.stx}ession.

Technically, a callable object will have a [**\_\_call\_\_**][p-dm-call] method, which is implicitly called when you call the [object]{.stx} using ‘normal’ syntax. For methods, this means:

&nbsp;&nbsp;&nbsp;&nbsp; [object]{.stx}**(** [args]{.stx} **)** &nbsp;&nbsp; ≡ &nbsp;&nbsp; [object]{.stx}**.\_\_call\_\_(** [object]{.stx}**,** [args]{.stx} **)**

For [built-in][p-fn] and [user-defined][p-st-def] functions, this becomes:

&nbsp;&nbsp;&nbsp;&nbsp; [ident]{.stx}**(** [args]{.stx} **)** &nbsp;&nbsp; ≡ &nbsp;&nbsp; [ident]{.stx}**.\_\_call\_\_(**[args]{.stx} **)**
...
Some functions prohibit passing *keyword arguments*, which is Python's terminology for ‘[named parameters][w-named-parms]’, although we prefer the term ‘named arguments’. For the above **foo** function, the three calls are all equivalent.
...
A function is ‘special kind of a class’ object; each function has its own attribute dictionary (**\_\_dict\_\_**), which is how Python implements [scope][w-scope]. Only functions and classes have scope, unlike language like the C-family where a compound statement block also forms a scope.
...
#### **py** — *Adding attributes to functions*
```py
def func (param):
   """
...
#### **py** — *Simple function definition with code that attaches an attribute*
```py
def func(arg):
...
There are three built-in functions that can deal with attributes: [**getattr**][p-fn-getattr] (get value of attribute, which is what ‘**.**’ does), [**`setattr`**][p-fn-setattr] (which is what the equal sign does) and [**`hasattr`**][p-fn-hasattr] (which can check if an attribute exists on an object).
...
#### **py** — *Function that uses its own attribute*
```py
def func(arg):
   print(f"arg = {arg}")
   if hasattr(func, "X"):
...
#### **py** — *Memoize results in expensive function*
```py
def slow_func(x):
...
#### **py** — *Function returning function objects*
```py
import random
def FuncFactory ():
   """
...
#### **py** — *Function with callable parameter*
```py
def TakeFunc (parm):
   """
...
#### **py** — *Playing with functions as values*
```py
...
Passing functions is so useful, that several functions in the Python standard library accepts functions as arguments (sometimes they are optional). Common and very useful examples are, the [**map** function][p-fn-map], and the [**filter** function][p-fn-filter].

#### **py** — *Passing functions to `map()` and `filter()` example*
```py
...
Note that both [**filter**][p-fn-filter] and [**map**][p-fn-filter] return *iterators*, and not a complete [**list**][p-fn-list] or [**tuple**][p-fn-tuple]. They must be used in an *iterable* context to actually perform the iterations.

Here is an trivial example of a user-defined function similar in operation to built-in functions like [**map**][p-fn-map] and [**filter**][p-fn-filter], in that it also takes a function as argument:

#### **py** — *Function taking ‘plugin function’ as argument*
```py
...
|:-----------------------------------------------------------|
| In Python3, the original built-in **reduce** and **apply** functions have been removed. If you require similar behaviour, use [**functools.reduce**][p-ft-reduce]. The closes equivalent for **apply**, can be found in the [**multiprocessing**][p-lib-multiproc] module. Unfortunately, you will still see many references to, and examples of, the old **reduce** and **apply** functions. You should translate that to Python3 manually. |

Here is a simple example using [**functools.reduce**][p-ft-reduce] to sum a sequence of values. For interest, we also passed a [**lambda**][p-ex-lambda] expression to the function.

#### **py** — *Example use of functools.reduce*
```py
...
#### **py** — *Multiplication factory function*
```py
...
Sometimes we have values in [**list**][p-fn-list]s or [**dict**][p-fn-dict]ionaries, where we want to pass their *items* as arguments to functions. Python provides a special syntax for *list argument unpacking* and *dictionary argument unpacking*.

## List Argument Unpacking

Assume you have a function called **func** that takes three arguments, or one that takes an arbitrary number of arguments like [**five**](#py-function-with-variable-number-of-arguments) above. And further assume you have a list of values like this:

#### **py** — *Arbitrary list of three values*
```py
lst = [11, 22, 33]
```
...
#### **py** — *Passing list items individually to a function*
```py
...
Python offers an alternative called *list argument unpacking* in the form: **\*lst**, where **lst** must be a [**list**][p-fn-list] or [**tuple**][p-fn-tuple]. This syntax can be used when passing arguments to a function that can take a variable number of arguments, or one that takes exactly that number of positional arguments produced by the unpacking.

#### **py** — *Unpacking a list as individual arguments #1*
```py
five(*lst)
```
```
...
The [**four**](#py-various-examples-of-passing-arguments) function above can take 3 arguments. Let us see how this works out:

#### **py** — *Unpacking a list as individual arguments #2*
```py
four(*lst)
```
```
...
By the same token, assume you have a function like [**seven**](#py-function-with-keyword-dictionary-parameter) above, which is defined to take a *dictionary parameter*. And assume you have a dictionary like **D** below:

#### **py** — *Simplistic dictionary for following examples*
```py
D = {'keya':'value A', 'keyb':123}
```

We can call [**seven**](#py-function-with-keyword-dictionary-parameter) by manually ‘unpacking’ the keyword arguments, which is again not a very robust approach. The Python alternative is *dictionary argument unpacking*, using the ‘__\*\*__[dict]{.stx}’ syntax, where [dict]{.stx} must be a [**dict**][p-fn-dict]ionary:

#### **py** — *Unpacking dictionary as individual keyword arguments*
```py
print("1)", end=" ")
...
We should thus be able to call [**eight**](#py-function-with-variable-and-keyword-parameters), which accepts both a variable number of arguments, and some keyword arguments. Yes we can, and just for fun, we also show you what it would look like if you had to manually *unpack* the `L` and `D` values:

#### **py** — *Unpacking a list and a dictionary as arguments*
```py
print("1)", end=" ")
...
The term [**lambda**][p-ex-lambda] is a more formal term for ‘[anonymous function][w-anon-func] expression’.

The [**lambda**][p-ex-lambda] keyword can be used to represent a function without a name (anonymous). Since a function is a value, the whole [**lambda**][p-fn-lambda] expression is a value. It can therefore be [**return**][p-st-return]ed from a function, passed to a function, or stored elsewhere. The basic syntax is as follows:

#### **Syntax** — *Lambda Expressions*
&nbsp;&nbsp;&nbsp;&nbsp; **lambda** [ [parm-list]{.stx} ] **:** [expr]{.stx}

The [parm-list]{.stx} does not have to be enclosed in parentheses. The [expr]{.stx}ession automatically results in: ‘[**return**][p-st-return] [expr]{.stx}’. There can only be one expression, so lambdas are convenient only for simple functionality. Anywhere you can use a [**lambda**][p-ex-lambda], you can use a function name instead. 

#### **py** — *Functions and lambdas in various combinations*
```py
def taker (f):
   """
...
Note that we could have passed **taker** any name, as long as the name referenced a value, with type **function**, or more generically: anything which is callable by the function call operator, including an [anonymous function][w-anon-func] expression ([**lambda**][p-ex-lambda]).
...
#### **py** — *Capturing ‘outer’ variables = closure*
```py
...
The interesting effect of the [**print**][p-fn-print] function call in **looper2**, is that the ‘work’ of the function, takes place in *reverse*, which makes recursion a technique to change the order in which actions take place.
...
The [**yield** statement][p-st-yield] turns a function into a [generator][p-gl-generator], meaning it returns an [iterator][p-tp-iterator]. Thus, *generator function* is a term to describe a function which contains one or more [**yield**][p-st-yield] statements.

The syntax is similar to [**return**][p-st-return], and in fact, [**yield**][p-st-yield] *does* return an [expr]{.stx}ession to the caller. The key difference is that [**yield**][p-st-yield] saves the state of the function, so that when it is called again, the complete state is restored, and execution continues with the *next* statement following the [**yield**][p-st-yield] statement.
...
The iterator returned by calling a generator function, or [**iter**][p-fn-iter]**(**[sequence]{.stx}**)**, can be passed to the built-in [**next**][p-fn-next] function to return each item in the logical sequence. It calls the **\_\_next\_\_** method present in all iterators.

Iteration is terminated when [**next**][p-fn-next] raises a **StopIteration** exception. We can simulate what a [**for**][p-st-for] statement does:

#### **py** — *Simulating a for loop*
```py
SEQ = [ 11, 22, 33, 44, 55 ]
...
Here is another example using a generator to calculate [fibonacci numbers][w-fibonacci] given a **first** and **last** arguments. For example, if **first** is **3**, it means the 3rd number in the sequence, and a **last** value of **10**, would mean the 10th number in the sequence. For good measure, we use also use a nested function: **nth_fibonacci** using [Binet's formula][w-fibo-binet].

#### **py** — *Fibonacci sequence with a generator*
```py
import math
...
Note that the number represented by the **stop** is not included, much like [**range**][p-fn-range]. The algorithm is relatively efficient, but a more complex recursive solution using [memoization][w-memoize] might be more performant at the expense of using more memory.
...
#### **py** — *Recursive Fibonacci with memoization*
```py
import math
...
Functions are [first-class][w-first-class] values, which means they can be passed as arguments, returned from functions, assigned to a variable, or stored as an item in a data structure like a [**list**][p-fn-list], [**tuple**][p-fn-tuple], [**dict**][p-fn-dict]ionary, or custom classes.
...
After the statement: **some\_func = decorate(some\_func)**, the *name* **some_func** is given a new value to reference: the return from **decorate**, which is **inner**. Its original code is not directly referenced any more — it became ‘decorated’.
...
#### **py** — *More generic decorator pattern*
```py
def decorator(func):
   def inner(*args, **kwds):
...
&nbsp;&nbsp;&nbsp;&nbsp; [ident]{.stx} **=** [decorator]{.stx}**(**[ident]{.stx}**)**
...
#### **py** — *Multiple decorations on one function*
```py
def decorate(func):
   def inner(*args, **kwds):
      print("<decoration>", end=" ")
...
The last statement ‘redecorated’ the **once** function two more times, without using the special **@**[decorator]{.stx} syntax. After this, **once** has the same behaviour as **thrice**.

Python provides a number functions designed as decorators, for example [**property**][p-fn-property], [**staticmethod**][p-fn-staticmethod], [**classmethod**][p-fn-classmethod], and several in the [**functools**][p-lib-functools] module.
